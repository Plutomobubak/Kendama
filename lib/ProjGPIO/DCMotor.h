...
public:
    QEncoder(gpio_num_t encA, gpio_num_t encB) {
        ...
        if (res != ESP_OK) {
            std::cerr << "Failed to create PCNT unit" << std::endl;
        }
        ...
        if (res != ESP_OK) {
            std::cerr << "Failed to add pulse counter watch point" << std::endl;
        }
        ...
        if (res != ESP_OK) {
            std::cerr << "Failed to register pulse counter event callbacks" << std::endl;
        }

        res = pcnt_unit_enable(_unit);
        if (res != ESP_OK) {
            std::cerr << "Failed to enable pulse counter unit" << std::endl;
        }

        res = pcnt_unit_start(_unit);
        if (res != ESP_OK) {
            std::cerr << "Failed to start pulse counter channel" << std::endl;
        }
    }
    ...
    int64_t getPos() {
        ...
        if (res != ESP_OK) {
            std::cerr << "Failed to get pulse counter count" << std::endl;
        }
