...
Radio RadioO;
static constexpr const uint8_t RADIO_BLE_ADV_PROP_TYPE = 0x80;

const Radio::Config Radio::DEFAULT_CONFIG = {
    .init_nvs = true,
...
Radio::Radio() {
    ...
    m_data_size = 0;
    m_data[0] = RADIO_BLE_ADV_PROP_TYPE;
...
void Radio::gapEventHandler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t* param) {
    auto& self = RadioO;
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        if ((d.ble_adv[0] & 0xC0) != RADIO_BLE_ADV_PROP_TYPE) {
            break;
        }
...
void Radio::onTimeout(TimerHandle_t timer) {
    auto& self = RadioO;
...
std::function<void(PacketInfo)> Radio::prepareCallbackLocked(PacketDataType dtype, const uint8_t* data, size_t len) {
...
void Radio::setGroup(uint8_t group) {
    if (group >= 16) {
        ESP_LOGE(TAG, "The group id must be in range <0;16)");
        return;
    }
...
uint8_t Radio::group() const {
    return m_data[0] & 0x0F;
}

esp_err_t Radio::address(esp_bd_addr_t out_address) const {
    uint8_t addr_type;
    return esp_ble_gap_get_local_used_addr(out_address, &addr_type);
}

void Radio::setData(PacketDataType dtype, const uint8_t* data, size_t len) {
    if (len == 0) {
        m_data_size = 0;
        return;
    }
...
void Radio::submitAdvertisingData() {
